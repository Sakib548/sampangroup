...
export default function RealEstateLocationMap({
  ...
  return (
    <section id="location-map" className={`py-24 relative ${containerClasses}`}>
      <div className="mx-auto w-full max-w-[1440px] px-6 sm:px-10 lg:px-16">
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-5 border border-[#183b2b]/15 p-8 bg-white flex flex-col justify-between space-y-6 shadow-sm">
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-2xl font-bold text-[#183b2b]">{projectName}</h3>
                <p className="text-xs text-[#183b2b]/75 mt-1 leading-relaxed">{address}</p>
              </div>

              <div className="p-4 bg-[#f3f6f2] border border-[#183b2b]/10 font-mono text-xs flex justify-between">
                <span className="text-[#183b2b]/60">GPS Position:</span>
                <span className="font-bold text-[#ca8a04]">{gpsCoordinates}</span>
              </div>

              {/* Landmark Distance Matrix */}
              <div className="space-y-3 pt-2">
                <span className="font-mono text-xs font-bold uppercase block text-[#183b2b]/60 border-b border-[#183b2b]/10 pb-2">
                  Proximity to Key Landmarks
                </span>
                
                {landmarks.map((lm, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs py-1.5 border-b border-[#183b2b]/10">
                    <span className="text-[#183b2b]/90">{lm.landmark}</span>
          ...
          <div className="lg:col-span-7 border border-[#183b2b]/15 relative min-h-[420px] overflow-hidden bg-neutral-200 shadow-sm">
            <iframe
