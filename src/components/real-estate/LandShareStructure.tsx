...
export default function LandShareStructure({
  ...
  return (
    <section id="land-share" className={`py-24 relative ${containerClasses}`}>
      <div className="mx-auto w-full max-w-[1440px] px-6 sm:px-10 lg:px-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12">
          <div className="border border-[#183b2b]/15 bg-white p-6 space-y-1 shadow-sm">
            <span className="text-xs font-mono uppercase text-[#183b2b]/60">Total Project Footprint</span>
            <p className="text-3xl font-mono font-bold text-[#ca8a04]">{totalLandArea}</p>
          </div>
          <div className="border border-[#183b2b]/15 bg-white p-6 space-y-1 shadow-sm">
            <span className="text-xs font-mono uppercase text-[#183b2b]/60">Total Shares Allotted</span>
            <p className="text-3xl font-mono font-bold text-[#183b2b]">{totalSharesCount}</p>
          </div>
          <div className="border border-[#183b2b]/15 bg-white p-6 space-y-1 shadow-sm">
            <span className="text-xs font-mono uppercase text-[#183b2b]/60">Legal Deed Registration</span>
            <p className="text-lg font-mono font-bold text-[#008f68] flex items-center gap-2 mt-2">
              <FaShieldAlt />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tiers.map((tier, idx) => (
            <div
              key={idx}
              className="border border-[#183b2b]/15 bg-white p-8 flex flex-col justify-between relative overflow-hidden shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <div className="space-y-4">
                ...
                <h3 className="text-2xl font-bold text-[#183b2b] leading-snug">{tier.title}</h3>

                <div className="p-4 bg-[#f3f6f2] border border-[#183b2b]/10 font-mono text-xs space-y-2">
                  <div className="flex justify-between text-[#183b2b]/80">
                    <span>Share Size:</span>
                    <span className="font-bold text-[#183b2b]">{tier.shareSize}</span>
                  </div>
                  <div className="flex justify-between text-[#183b2b]/80">
                  ...
                  <div className="flex justify-between text-[#183b2b]/80">
                    ...
                    <span className="font-bold text-[#183b2b]">{tier.deedRegistration}</span>
                  </div>
                </div>

                <div className="space-y-2 pt-2">
                  {tier.keyBenefits.map((b, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs text-[#183b2b]/90">
              ...
              <div className="pt-6 mt-6 border-t border-[#183b2b]/15">
                <a
                  href="#site-visit"
