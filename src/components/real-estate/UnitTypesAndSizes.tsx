...
export default function UnitTypesAndSizes({
  ...
  return (
    <section id="unit-types" className={`py-24 relative ${containerClasses}`}>
      <div className="mx-auto w-full max-w-[1440px] px-6 sm:px-10 lg:px-16">
        ...
        {/* Filter Tabs */}
        {categories.length > 2 && (
          <div className="flex flex-wrap gap-2 mb-12 border-b border-[#183b2b]/15 pb-4">
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredUnits.map((unit) => (
            <div
              key={unit.id}
              className="border border-[#183b2b]/15 bg-white p-8 flex flex-col justify-between space-y-6 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group"
            >
              <div className="space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <span className="font-mono text-xs text-[#183b2b]/60 font-bold flex items-center gap-1">
                    <FaCompass className="text-[#ca8a04]" />
                    {unit.orientation}
                  </span>
                </div>

                <h3 className="text-2xl font-bold text-[#183b2b] leading-snug">{unit.name}</h3>

                {/* Specs Strip */}
                <div className="grid grid-cols-3 gap-2 p-4 bg-[#f3f6f2] border border-[#183b2b]/10 font-mono text-xs text-center">
                  <div>
                    <span className="block text-[#183b2b]/60 text-[10px] uppercase">Size</span>
                    <span className="font-bold text-[#ca8a04]">{unit.sizeSqFt}</span>
                  </div>
                  <div>
                    <span className="block text-[#183b2b]/60 text-[10px] uppercase">Bedrooms</span>
                    <span className="font-bold flex items-center justify-center gap-1 text-[#183b2b]">
                      <FaBed className="text-xs" /> {unit.bedrooms}
                    </span>
                  </div>
                  <div>
                    <span className="block text-[#183b2b]/60 text-[10px] uppercase">Baths</span>
                    <span className="font-bold flex items-center justify-center gap-1 text-[#183b2b]">
                ...
                <div className="space-y-2 pt-2 text-xs">
                  {unit.highlights.map((h, i) => (
                    <div key={i} className="flex items-center gap-2 text-[#183b2b]/90">
              ...
              {/* Card Footer */}
              <div className="pt-6 border-t border-[#183b2b]/15 flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-mono uppercase block text-[#183b2b]/60">Estimated Price</span>
