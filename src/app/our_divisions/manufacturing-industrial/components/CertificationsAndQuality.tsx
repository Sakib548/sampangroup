...
import { FaAward, FaShieldAlt, FaCheckCircle, FaFlask } from "react-icons/fa";
...
export interface CertificationsAndQualityProps {
  title?: string;
  subtitle?: string;
...
export default function CertificationsAndQuality({
  title = "Quality Standards & Official Certifications",
  subtitle = "Our products undergo batch lab testing to ensure compliance with national BSTI standards, ISO 9001 quality management, and eco-friendly specifications.",
  certifications,
  bgTheme = "about-ivory",
  accentColor = "#047857",
}: CertificationsAndQualityProps) {
  ...
  return (
    <section id="certifications" className={`py-24 relative overflow-hidden ${containerClasses}`}>
          ...
          {certifications.map((cert, idx) => (
            ...
            >
              <div className="space-y-4">
                ...
                <h3 className="text-xl font-bold text-current leading-snug">{cert.title}</h3>

                <div className="p-3 bg-[#f3f6f2] border border-current/10 font-mono text-xs">
                  <span className="opacity-60 block text-[10px]">Cert / Memo No:</span>
                  <span className="font-bold text-[#183b2b]">{cert.referenceNo}</span>
                </div>

                <p className="text-xs opacity-75 leading-relaxed font-normal">
                  {cert.standardDetails}
                </p>
              </div>
